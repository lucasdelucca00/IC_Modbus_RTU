...
/**
  ...
  *
  ******************************************************************************
  NEW EXPRESSION: batt, modbus_regs, batt_rx_buf, batt_err_code, batt.data.voltage_dV
  */
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  // Inicia a memória ANTES do FreeRTOS assumir o controle
  modbus_init(20);
  sunspec_init(20);

  //  Coloca a porta do PC em modo de escuta!
  rs485_rx(GPIOB, GPIO_PIN_1);
  /* USER CODE END 2 */
  ...
  osThreadDef(batteryTask, StartBatteryTask, osPriorityAboveNormal, 0, 512);
  batteryTaskHandle = osThreadCreate(osThread(batteryTask), NULL);

  /* definition and creation of cliTask */
  osThreadDef(cliTask, StartCLITask, osPriorityLow, 0, 512);
...
{
    // Apenas o QModBus do PC usa interrupção
    if (huart->Instance == USART3)
    {
        modbus_rtu_rx_complete(Size);
    }
}

void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    // Ignora erros de ruído
}
/* USER CODE END 4 */
...
void StartModbusTask(void const * argument)
{
  /* USER CODE BEGIN 5 */
	...
	{
		modbus_poll();
	    osDelay(10); // Pausa 10ms (não sufocar o RTOS)
	}
  /* USER CODE END 5 */
}
...
void StartBatteryTask(void const * argument)
{
  /* USER CODE BEGIN StartBatteryTask */
	  ...
	  for(ever)
	  {
	      ...
	      // O LED pisca aqui para provar que a Task da Bateria esta funfando
	      HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
	      osDelay(500); // RTOS, me acorde daqui a meio segundo.
	  }
